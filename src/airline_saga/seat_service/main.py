...
from airline_saga.seat_service.models import (
    Flight, BlockSeatRequest, ReleaseSeatRequest, SeatStatus
)
...
app = FastAPI(title="Seat Service", description="Service for managing flight seats", lifespan=setup_teardown_lifespan)
...
async def get_seat_of_flight(
    ...
    if not seat:
        raise SeatNotFoundException(f"Seat {seat_number} not found in flight {flight_number}")
...
async def release_seat(request: ReleaseSeatRequest):
    ...
    if booking_id not in blocked_seats:
        raise SeatNotFoundException(
            f"No blocked seat found for booking {booking_id}",
            booking_id=booking_id
        )
